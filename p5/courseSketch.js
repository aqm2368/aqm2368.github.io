let shaders;
let squares = [];
let size = 124; let cols = window.screen.width / size; let rows = window.screen.height / size;
let side1;
let startDepth = -100;
let mouseRange = 80;
...
function setup() {
    ...
    size = 124 * 1.25 / (window.devicePixelRatio);
...
class Rectangle {
    ...
    display() {
        ...
        if((mouseX - width/2 - this.x * 1.00)*(mouseX - width/2 - this.x * 1.00) + (mouseY - height/2 - this.y)*(mouseY - height/2 - this.y) < mouseRange*mouseRange) {
            this.z = lerp(this.z, startDepth + size * 2 - size, 0.05);
